...
// 경고창 출력 + 포커스 이동 + 값 삭제
// -> 반복되는 코드를 줄임
function alertAndFocus(element, str){
    alert(str);
    input.focus();
    input.value="";
}
...
if(memberDeleteForm!=null){ // 탈퇴form 요소가 존재할 때
    ...
    memberDeleteForm.addEventListener("submit", function(event){
        ...
        // 회원 탈퇴에 사용되는 input 요소 얻어오기
        const memberPw=document.getElementById("memberPw");

        if(memberPw.value.trim().length==0){ // 비밀번호 미작성

            alert("비밀번호를 입력해주세요."); // alert 출력후 포커스 이동
            memberPw.focus(); 
            memberPw.value="";
            event.preventDefault(); //  기본 이벤트 제거
            return;
        ...
        // 2) 동의 체크가 되지 않은 경우
        //  -> "탈퇴를 동의하시면 체크를 눌러주세요." alert 출력 후
        //  포커스 이동
